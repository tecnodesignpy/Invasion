...
var GoogleMap = (function () {
	...
	_createClass(GoogleMap, [{
		...
		value: function init() {
			...
			var map = new google.maps.Map(this.canvasMap, mapOptions);
			marker.setMap(map);
		...
		value: function setPosition() {
			...
			arrProto.forEach.call(pages, function (page, index) {
				...
				if (page.classList.contains('current-page')) {
					_this7.hasCurrent = true;
					// this.goTo( index, 0 );
					_this7.currentPage = index;
				}
			});
  ...
  */
		...
		value: function clickLoadPage(menu) {
			...
			menu.addEventListener('click', function (event) {
				...
				if (page.classList.contains('ajax-page')) {
					...
					_this22.contentPageBlock = page.querySelector('.ajax-content');
					if (!_this22.contentPageBlock) {
						...
						_this22.contentPageBlock = mainDiv;

						_this22.scrollPageBlock = scrollDiv;

						var menu = page.querySelector('.ajax-menu');
						if (menu) {
							menu.parentNode.appendChild(_this22.contentPageBlock);
						} else {
							console.error('Ajax page must contain "ajax-menu" , and a with data-action!!!');
						}
					}

					_this22.scrollPageBlock = _this22.contentPageBlock.querySelector('.scroll');
					...
					_this22.animateHeight = true;
					_this22.sendRequest(action, galleryId, target);
		...
		value: function sendRequest(action) {
			...
			xhr.onreadystatechange = function () {
				...
				if (_this23.contentPageBlock) {
					_this23.contentPageBlock.classList.remove('ajax-loaded');
				}

				if (_this23.mainBlock) {
					if (_this23.mainBlock.classList.contains('ajax-page-post')) {
						if (_this23.contentPageBlock) {
							_this23.contentPageBlock.classList.add('ajax-loaded');
						}
					}
				}

				var page = pages.contentPage[pages.currentPage];
				_this23.pageTitle = page.querySelector('.page-title');
				var title = '';
				if (_this23.pageTitle) {
					title = _this23.pageTitle.outerHTML;
				}
				...
				if (xhr.readyState === 4 && xhr.status === 200) {
					...
					if (windowWidth < 992) {
						var parent;
						parent = _this23.contentPageBlock.parentNode;
					...
					setTimeout(function () {
						...
						if (_this23.contentPageBlock) {
							var _parent2 = _this23.contentPageBlock.parentNode;
							if (_parent2) {
						...
						if (galleryId) {
							...
							_this23.mainBlock.classList.add('current-page');
						}

						_this23.scrollPageBlock.scrollTop = 0;

						_this23.scrollPageBlock.innerHTML = title + xhr.responseText;
						scroll.init(_this23.scrollPageBlock.parentNode);

						if (windowWidth >= 992) {
							if (_this23.mainBlock) {
								var ajaxTitle = _this23.mainBlock.querySelector('.ajax-gallery-title');
								if (ajaxTitle) {
									centerImage({
										block: ajaxTitle,
										nameClass: 'img'
						...
						var imgs = _this23.contentPageBlock.querySelectorAll('img'),
						...
						if (imgs.length > 0) {
							...
							arrProto.forEach.call(imgs, function (img) {
								...
								newImg.onload = function () {
									...
									if (count >= imgsLength) {
										...
										_this23.contentPageBlock.classList.add('ajax-loaded');
										if (_this23.mainBlock) {
											_this23.mainBlock.classList.add('ajax-page-mobile--loaded');
										}
										if (target) {
											target.classList.remove('loading');
										}

										var _gallery = _this23.contentPageBlock.querySelectorAll('.gallery');
								...
								newImg.onerror = function () {
									...
									if (count >= imgsLength) {
										...
										var _gallery2 = _this23.contentPageBlock.querySelectorAll('.gallery');
							...
							var _gallery3 = _this23.contentPageBlock.querySelectorAll('.gallery');
						...
						if (_this23.contentPageBlock) {
							if (_this23.contentPageBlock.parentNode) {
								// this.contentPage.classList.add('loading');

								// if ( this.firstLoadContent ) {
								var menu = _this23.contentPageBlock.parentNode.querySelector('.ajax-menu');
								if (menu) {

									_this23.setActiveElement(action, menu);
								...
								_this23.firstLoadContent = false;
								// }
							}
						}
		...
		value: function deletePage(target) {
			...
			setTimeout(function () {
				if (_this24.mainBlock) {
					if (_this24.mainBlock.parentNode) {
						_this24.mainBlock.parentNode.removeChild(_this24.mainBlock);
						_this24.mainBlock = null;
					}
				}
				var nav = document.querySelector('.navigation');
				if (nav) {
   ...
   */
		...
		value: function setActiveElement(action, menu) {
			...
			var ajaxMenu = menu ? menu : this.contentPage.querySelector('.ajax-menu');
			...
			for (var i = 0; i < linkLength; i++) {
				if (link[i].getAttribute('data-action') === action) {
					link[i].classList.add('active');
					link[i].classList.add('loading');
					break;
				}
			}
	...
	_createClass(Accordeon, [{
		...
		value: function init() {
			...
			window.addEventListener('resize', function () {
				...
				if (_this25.lastOpen) {
					var content = _this25.lastOpen;
					if (content.classList.contains('show')) {
						content.classList.add('show');
						content.parentNode.classList.add('open');
						content.parentNode.style.height = content.offsetHeight + 'px';
						if (_this25.lastOpen && _this25.lastOpen !== content) {
							_this25.lastOpen.classList.remove('show');
							_this25.lastOpen.parentNode.classList.remove('open');
							_this25.lastOpen.parentNode.style.height = '';
						}

						_this25.lastOpen = content;
					}
				}
			});
		}
	...
	_createClass(NavigationCarousel, [{
		...
		value: function closeFullMenu() {
			...
			setTimeout(function () {
				...
				if (_this27.isOpenedMenu) {
					return false;
				}

				_this27.navMenu.parentNode.style.height = _this27.parentHeight + 'px';
				if (_this27.liLength % 2 === 0) {
					_this27.navMenu.parentNode.style.top = '';
				}

				_this27.navMenu.style.WebkitTransition = '-webkit-transform ' + 0 + 'ms';
				_this27.navMenu.style.transition = '-webkit-transform ' + 0 + 'ms';
				transition(_this27.navMenu, 'transform ' + 0 + 'ms');

				// change translateX
				if (/MSIE 9/i.test(navigator.userAgent) || /MSIE 10/i.test(navigator.userAgent) || /rv:11.0/i.test(navigator.userAgent)) {
					transform(_this27.navMenu, 'translateY(' + -_this27.liPosition[_this27.currentPosition] + 'px)');
				} else {
					transform(_this27.navMenu, has3d ? 'translate3d(0, ' + -_this27.liPosition[_this27.currentPosition] + 'px, 0)' : 'translateY(' + -_this27.liPosition[_this27.currentPosition] + 'px)');
				}

				// setTimeout( () => {
				if (_this27.setPosition) {

					for (var _i12 = 0; _i12 < _this27.allLiLength; _i12++) {
						_this27.lis[_i12].classList.remove('main-active');
						_this27.lis[_i12].classList.remove('second-active');
						_this27.lis[_i12].classList.remove('last-active');
					}

					_this27.prevPosition = _this27.currentPosition;
					_this27.currentPosition = _this27.tempPosition;

					_this27.goTo(_this27.currentPosition, 500, true);

					_this27.setPosition = false;
				}

				_this27.isOpenFullMenu = false;
   ...
   */
		...
		value: function MouseTouchMove() {
			...
			var moveEnd = function moveEnd(event) {
				...
				_this28.prevPosition = curPos;

				if (lastPos !== undefined) {
					_this28.currentPosition = lastPos;
				} else {
					_this28.currentPosition = curPos;
				}

				_this28.isMove = false;

				_this28.changeClass();

				_this28.goTo(_this28.currentPosition);
			...
			var moveMenu = function moveMenu(event) {
				...
				if (position) {
					...
					} else if (position < _this28.positionFull) {

						// curPos = Math.ceil((curPos) / 2);
						curPos = lastPos - _this28.liLength - 1;
				...
				position = -_this28.liPosition[curPos] + changes;

				transform(_this28.navMenu, has3d ? 'translate3d(0, ' + position + 'px, 0)' : 'translateY(' + position + 'px)');

				_this28.lastPosition = -position;

				var itemChange = changes / _this28.liHeight;
				...
				if (!tmpLastPos) {
					...
					if (lastPos >= _this28.liLength) {
						tmpPos = lastPos - _this28.liLength;
					} else {
						tmpPos = lastPos;
					}

					for (var _i13 = 0; _i13 < 3; _i13++) {
						if (_this28.lis[_i13 * _this28.liLength + tmpPos + 2]) {
							_this28.lis[_i13 * _this28.liLength + tmpPos + 2].classList.add('main-active');
							_this28.lis[_i13 * _this28.liLength + tmpPos + 2].classList.remove('last-active');
							_this28.lis[_i13 * _this28.liLength + tmpPos + 2].classList.remove('second-active');
						}
						if (_this28.lis[_i13 * _this28.liLength + tmpPos]) {
							_this28.lis[_i13 * _this28.liLength + tmpPos].classList.add('last-active');
							_this28.lis[_i13 * _this28.liLength + tmpPos].classList.remove('main-active');
							_this28.lis[_i13 * _this28.liLength + tmpPos].classList.remove('second-active');
						}
						if (_this28.lis[_i13 * _this28.liLength + tmpPos + 4]) {
							_this28.lis[_i13 * _this28.liLength + tmpPos + 4].classList.add('last-active');
							_this28.lis[_i13 * _this28.liLength + tmpPos + 4].classList.remove('main-active');
							_this28.lis[_i13 * _this28.liLength + tmpPos + 4].classList.remove('second-active');
						}
						if (_this28.lis[_i13 * _this28.liLength + tmpPos + 1]) {
							_this28.lis[_i13 * _this28.liLength + tmpPos + 1].classList.add('second-active');
							_this28.lis[_i13 * _this28.liLength + tmpPos + 1].classList.remove('main-active');
							_this28.lis[_i13 * _this28.liLength + tmpPos + 1].classList.remove('last-active');
						}
						if (_this28.lis[_i13 * _this28.liLength + tmpPos + 3]) {
							_this28.lis[_i13 * _this28.liLength + tmpPos + 3].classList.add('second-active');
							_this28.lis[_i13 * _this28.liLength + tmpPos + 3].classList.remove('main-active');
							_this28.lis[_i13 * _this28.liLength + tmpPos + 3].classList.remove('last-active');
						}
					}
			...
			var onMenu = function onMenu(event) {
				...
				_this28.isMove = true;
				_this28.touchMove = true;
				curPos = _this28.currentPosition;
		...
		value: function animateMenu(currentPosition, endPosition) {
			...
			this.animate({
				...
				draw: function draw(progress) {
					...
					transform(_this29.navMenu, has3d ? 'translate3d( 0, ' + -progressX + 'px, 0)' : 'translateY(' + -progressX + 'px)');

					_this29.lastPosition = progressX;

					if (progress >= 1) {
						_this29.isAnimate = false;
						_this29.onTransitionEnd();
					}
				}
			});
		}
		...
		value: function resize() {
			...
			window.addEventListener('resize', function () {
				...
				t = setTimeout(function () {
					...
					_this30.liPosition = [];
					_this30.lis[0].style.height = '';

					_this30.navMenu.style.height = _this30.menuHeight + 'px';

					_this30.liHeight = _this30.lis[0].clientHeight;

					_this30.menuHeight = _this30.liHeight * _this30.liLength * 3;

					for (var _i14 = 0; _i14 < _this30.allLiLength; _i14++) {
						_this30.lis[_i14].style.height = _this30.liHeight + 'px';
						_this30.liPosition.push(_this30.liHeight * _i14);
					}

					_this30.parentHeight = _this30.liHeight * _this30.countVisibleItem;

					_this30.navMenu.parentNode.style.height = _this30.parentHeight + 'px';

					_this30.fullHieght = _this30.liHeight * _this30.liLength;

					_this30.currentPosition = _this30.liLength - 1;

					var z = Math.floor(_this30.countVisibleItem / 2);
					var currentPage = pages.currentPage;
					if (currentPage >= pages.contentPageLength) {
						currentPage = 0;
					}
					var page = pages.contentPage[currentPage];
					var id = page.getAttribute('id');
					var link = _this30.lis[_this30.currentPosition + z].querySelector('a');
					var hash = link.getAttribute('href');
					hash = hash.replace('#', '');

					var i = _this30.currentPosition + z;
					if (id !== hash || !hash) {
						for (; i < _this30.allLiLength; i++) {
							var tmp = _this30.lis[i].querySelector('a');
							var tmpHash = tmp.getAttribute('href');
							hash = tmpHash.replace('#', '');
							if (id === hash) {
								break;
						...
						_this30.currentPosition = i - Math.floor(_this30.countVisibleItem / 2);
					} else {
						_this30.currentPosition;
					}

					_this30.navMenu.style.WebkitTransition = '-webkit-transform ' + 0 + 'ms ';
					_this30.navMenu.style.transition = '-webkit-transform ' + 0 + 'ms ';
					transition(_this30.navMenu, 'transform ' + 0 + 'ms ');

					// change translateX
					transform(_this30.navMenu, has3d ? 'translate3d(0, ' + -_this30.liPosition[_this30.currentPosition] + 'px, 0)' : 'translateY(' + -_this30.liPosition[_this30.currentPosition] + 'px)');

					_this30.positionFull = -_this30.liPosition[_this30.allLiLength - _this30.countVisibleItem];
...
var Slider = (function () {
	function Slider() {
		...
		arrProto.forEach.call(this.items, function (item) {
			...
			item.style.transform = _this31.animationPath[rand];
			item.style.webkitTransform = _this31.animationPath[rand];
		});
	...
	_createClass(Slider, [{
		...
		value: function init() {
			...
			this.animateSlide = function () {
				...
				_this32.items[prev].style.transitionDuration = 0 + 'ms';
				_this32.items[prev].style.webkitTransitionDuration = 0 + 'ms';
				_this32.items[prev].style.transitionDelay = _this32.speed + 'ms';
				_this32.items[prev].style.webkitTransitionDelay = _this32.speed + 'ms';
				// setTimeout(()=>{
				var rand = Math.floor(Math.random() * (4 - 0) + 0);
				_this32.items[prev].style.transform = _this32.animationPath[rand];
				_this32.items[prev].style.webkitTransform = _this32.animationPath[rand];
				_this32.items[prev].classList.remove('active');
				// },0);

				_this32.items[i].style.transitionDuration = _this32.speed + 'ms';
				_this32.items[i].style.webkitTransitionDuration = _this32.speed + 'ms';
				_this32.items[i].style.transitionDelay = 0 + 'ms';
				_this32.items[i].style.webkitTransitionDelay = 0 + 'ms';
				_this32.items[i].classList.add('active');
			};
...
var FullImage = (function () {
	...
	_createClass(FullImage, [{
		...
		value: function init(target) {
			...
			setTimeout(function () {
				_this34.createFullImage(images);
			}, 0);
		...
		value: function createFullImage(images) {
			...
			arrProto.forEach.call(images, function (image, index) {
				if (image.classList.contains('active')) {
					_this35.currentImage = index;
				}

				var cloneImage = image.cloneNode(true);
				_this35.images.push(cloneImage);
				var li = document.createElement('li');

				// change on resize
				li.style.width = _this35.mainWidth + 'px';
				li.style.height = _this35.mainHeight + 'px';

				// this.centerImage ( cloneImage );
				// this.containImage( cloneImage );

				if (!_this35.contain) {
					_this35.centerImage(cloneImage);
				} else {
					_this35.containImage(cloneImage);
				}

				li.appendChild(cloneImage);
				_this35.fullImageUl.appendChild(li);
				widthUl = widthUl + _this35.mainWidth;

				_this35.position.push(index * _this35.mainWidth);
			});
			...
			this.fullImageWrapper.onmousemove = function () {
				_this35.fullImageWrapper.classList.add('onmouse-move');
				clearTimeout(timer);
				timer = setTimeout(function () {
					_this35.fullImageWrapper.classList.remove('onmouse-move');
				}, 1000);
			};

			this.buttonClose.onmouseover = function () {
				_this35.fullImageWrapper.classList.add('onmouse-over');
			};
			this.buttonClose.onmouseleave = function () {
				_this35.fullImageWrapper.classList.remove('onmouse-over');
			};

			this.buttonLeft.onmouseover = function () {
				_this35.fullImageWrapper.classList.add('onmouse-over');
				_this35.fullImageWrapper.classList.add('onmouse-over-left');
			};
			this.buttonLeft.onmouseleave = function () {
				_this35.fullImageWrapper.classList.remove('onmouse-over');
				_this35.fullImageWrapper.classList.remove('onmouse-over-left');
			};

			this.buttonRight.onmouseover = function () {
				_this35.fullImageWrapper.classList.add('onmouse-over');
				_this35.fullImageWrapper.classList.add('onmouse-over-right');
			};
			this.buttonRight.onmouseleave = function () {
				_this35.fullImageWrapper.classList.remove('onmouse-over');
				_this35.fullImageWrapper.classList.remove('onmouse-over-right');
			};

			setTimeout(function () {
				_this35.fullImageWrapper.classList.add('loaded');
				_this35.fullImageLoaded = true;
		...
		value: function centerImage(img) {
			...
			newImage.onload = function () {
				...
				var imgWidth = _this36.mainHeight * proportion;
				var imgHeight = _this36.mainHeight;
				var top = 0;
				var left = 0;

				if (imgWidth < _this36.mainWidth) {
					imgWidth = _this36.mainWidth;
					imgHeight = imgWidth / proportion;
				}

				if (_this36.mainHeight < imgHeight) {
					top = -(imgHeight - _this36.mainHeight) / 2;
				}

				if (_this36.mainWidth < imgWidth) {
					left = -(imgWidth - _this36.mainWidth) / 2;
				}
		...
		value: function closeImage() {
			...
			this.contain = false;
			setTimeout(function () {
				_this37.target.classList.remove('active');
				_this37.fullImageWrapper.parentNode.removeChild(_this37.fullImageWrapper);
				_this37.fullImageIsExist = false;
			}, 300);
		}
	}, {
		key: 'fullImageResize',
		value: function fullImageResize() {
			...
			window.addEventListener('resize', function () {
				if (!_this38.fullImageIsExist) {
					return false;
				}

				_this38.mainWidth = parseInt(window.innerWidth * 0.8, 10);
				_this38.mainHeight = parseInt(window.innerHeight * 0.8, 10);
				var widthUl = 0;

				_this38.position = [];

				// change on resize
				_this38.fullImageBlock.style.width = _this38.mainWidth + 'px';
				_this38.fullImageBlock.style.height = _this38.mainHeight + 'px';

				// create li with iamge
				arrProto.forEach.call(_this38.images, function (image, index) {

					if (image.classList.contains('active')) {
						_this38.currentImage = index;
					}

					// change on resize
					image.parentNode.style.width = _this38.mainWidth + 'px';
					image.parentNode.style.height = _this38.mainHeight + 'px';

					if (!_this38.contain) {
						_this38.centerImage(image);
					} else {
						_this38.containImage(image);
					}

					widthUl = widthUl + _this38.mainWidth;

					_this38.position.push(index * _this38.mainWidth);
				});

				// change on resize
				_this38.fullImageUl.style.width = widthUl + 'px';
				_this38.fullImageUl.style.height = _this38.mainHeight + 'px';

				_this38.goTo(_this38.currentImage);
			});
		}
	}, {
		key: 'swipe',
	...
	_createClass(BlogList, [{
		...
		value: function getList() {
			...
			xhr.onreadystatechange = function () {
				...
				if (xhr.readyState === 4 && xhr.status === 200) {
					_this40.list = JSON.parse(xhr.responseText);
					_this40.showPost();
				}
			};
		}
	...
	_createClass(SubULMenu, [{
		...
		value: function showSubMenu(target) {
			...
			setTimeout(function () {
				...
				_this41.ul.classList.add('hide-menu');
				_this41.menuWrapper.classList.add('hide-menu');

				// this.menuWrapper.style.display = 'none';
				_this41.subUl.classList.add('show-menu');

				_this41.div.appendChild(_this41.subUl);

				_this41.allUl.push(_this41.subUl);
				_this41.li.push(_this41.targetParent);

				_this41.div.classList.remove('sub-nav-hide');
			}, 300);
		}
	}, {
		key: 'closeSubMenu',
		value: function closeSubMenu() {
			...
			setTimeout(function () {
				...
				_this42.allUl[ulLength - 1].classList.remove('show-menu');

				if (_this42.allUl[ulLength - 2]) {
					_this42.allUl[ulLength - 2].classList.remove('hide-menu');
				}
				_this42.li[liLength - 1].appendChild(_this42.allUl[ulLength - 1]);

				if (ulLength === 1) {
					// this.menuWrapper.classList.remove('hide-menu');
					_this42.navMenu.classList.remove('hide-menu');
					// this.menuWrapper.style.display = '';
					_this42.menuWrapper.classList.remove('hide-menu');
